...
ndpFramework.controller('SectorController',
    ...
    $scope.getAnalyticsData = function(){
        ...
        if( !$scope.model.selectedSector || !$scope.model.selectedSector.organisationUnits || $scope.model.selectedSector.organisationUnits.length === 0 ){
            NotificationService.showNotifcationDialog($translate.instant("error"), $translate.instant("missing_sector"));
        }
        ...
        if( $scope.model.dataElementGroup && $scope.model.dataElementGroup.length > 0 && $scope.model.selectedPeriods.length > 0){
            analyticsUrl += '&filter=ou:'+ $.map($scope.model.selectedSector.organisationUnits, function(ou){return ou.id;}).join(';');
            analyticsUrl += '&displayProperty=NAME&includeMetadataDetails=true';
