...
.factory('OrgUnitFactory', function($http, DHIS2URL, $q, SessionStorageService) {
    ...
    return {
        getChildren: function(uid){
            if( orgUnit !== uid ){
                orgUnitPromise = $http.get( DHIS2URL + '/organisationUnits/'+ uid + '.json?fields=id,path,organisationUnitGroups[attributeValues[value,attribute[code]]],children[id,displayName,level,children[id]]&paging=false' ).then(function(response){
                    orgUnit = uid;
        ...
        getViewTreeRoot: function(){
            if(!rootOrgUnitPromise){
                var url = DHIS2URL + '/me.json?fields=organisationUnits[id,displayName,level,path,organisationUnitGroups[attributeValues[value,attribute[code]]],children[id,displayName,level,children[id]]],dataViewOrganisationUnits[id,displayName,level,path,children[id,displayName,level,children[id]]]&paging=false';
                rootOrgUnitPromise = $http.get( url ).then(function(response){
                    ...
                    //console.log('response.data view tree:  ', response.data);
                    return response.data;
                });
            }
